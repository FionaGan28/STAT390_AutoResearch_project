...
Run the final experiment: load locked test set data, train agent's model, evaluate, log result.
"""
import sys
import time
import os
...
def main():
    ...
    # reads the locked test set instead of the validation split
    test_df = pd.read_csv(os.path.join(PROJECT_ROOT, 'data', 'locked_test', 'test_sample.csv'))
    test_df['skills_list'] = test_df['skills_list'].apply(ast.literal_eval)

    print(f"Data: {len(train_df)} train.")
    ...
    # 3. Evaluate
    print("Evaluating...")
    accuracy = evaluate_model(predict_fn, test_df)
